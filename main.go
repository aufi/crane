...
import (
	...
	transform_apply "github.com/konveyor/crane/cmd/transform-apply"
	transform_prepare "github.com/konveyor/crane/cmd/transform-prepare"
...
func main() {
	...
	root.AddCommand(transform.NewTransformCommand(f)) // Only for list-plugins and optionals subcommands
	root.AddCommand(transform_prepare.NewTransformPrepareCommand(f))
	root.AddCommand(transform_apply.NewTransformApplyCommand(f))
	root.AddCommand(skopeo_sync_gen.NewSkopeoSyncGenCommand(f))
	root.AddCommand(validate.NewValidateCommand(f))
	root.AddCommand(apply.NewApplyCommand(f)) // Kept for backward compatibility
